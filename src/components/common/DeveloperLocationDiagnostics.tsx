...
export const DeveloperLocationDiagnostics: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-4 left-4 z-50 font-mono text-xs max-w-sm sm:max-w-md w-full select-none">
      {/* Floating Toggle Pill */}
      <div className="flex items-center gap-2">
        <button
          id="dev-diagnostics-toggle-btn"
          onClick={() => setIsOpen(!isOpen)}
          className={`px-3 py-1.5 rounded-full flex items-center gap-2 font-bold shadow-lg border backdrop-blur-md transition-all cursor-pointer ${
            centralLocation.latitude 
              ? 'bg-stone-900/90 text-emerald-400 border-stone-700 hover:bg-stone-900' 
              : 'bg-amber-950/90 text-amber-300 border-amber-800/60 hover:bg-amber-950'
          }`}
        ...
        >
          ...
          <span className="text-[11px] font-sans tracking-wide">
            GPS Diagnostics {centralLocation.latitude ? `(${centralLocation.latitude.toFixed(3)}°, ${centralLocation.longitude?.toFixed(3)}°)` : '[No GPS]'}
          </span>
      ...
      {isOpen && (
        <div className="mt-2 bg-stone-950/95 border border-stone-800 text-stone-300 rounded-2xl shadow-2xl p-4 space-y-3 backdrop-blur-md animate-in fade-in slide-in-from-bottom-2 duration-150">
